#!/usr/bin/env python3
"""提出物バリデータ（静的チェック + 動的スモークテスト）

VLA コンペの提出物（HTTP ポリシーサーバー方式）を、フル評価（~1000秒）を
走らせる前に軽量に検証する。参加者はローカルで自己診断でき、運営は採点
スクリプトの unzip 直後ゲートとして同じ実装を呼ぶ（1ソース・2用途）。

    # 参加者（ローカル自己診断）
    python validate_submission.py my_submission.zip            # 静的 + 動的スモーク
    python validate_submission.py my_submission.zip --static   # 静的のみ（起動しない）
    python validate_submission.py ./my_submission_dir          # 展開済みディレクトリでも可

    # 運営（採点前ゲート）: evaluate.py から validate_zip() / safe_extract() を呼ぶ

検査項目（実装案 §1〜§3）:
  §1 静的  : zip 健全性 / Zip Slip / zip bomb / サイズ上限 / 必須ファイル /
             ルート直下判定 / policy_server.py の構文(ast) / 不要物警告 /
             エンドポイント文字列
  §2 依存  : requirements.txt の構文 / 外部ソース指定の禁止 / (任意) pip dry-run
  §3 動的  : サーバー起動→/health→/reset→/act(実形状ダミーobs)→
             float32 shape(7,)・NaN/Inf・値域 / レイテンシ計測 / 決定性

サンドボックス（§4: ネットワーク遮断・FS 分離・リソース制限）は実行環境側
（Omnicampus / Docker）の責務であり、本ツールの対象外。
"""
...
# ------------------------------------------------------------------
# 制約値（pipeline/config.py:SubmissionConstraints と一致させること）
# ------------------------------------------------------------------
MAX_ZIP_SIZE_MB = 20 * 1024          # zip 本体上限: 20GB（2026-07-12 決定）
MAX_EXTRACTED_SIZE_MB = 40 * 1024    # 展開後合計上限: 40GB（zip bomb 対策）
MAX_ENTRIES = 200_000                # エントリ数上限（zip bomb 対策）
MAX_COMPRESSION_RATIO = 300          # 圧縮率上限（zip bomb 対策）

REQUIRED_FILES = ["policy_server.py", "requirements.txt"]
REQUIRED_ENDPOINTS = ["/health", "/reset", "/act"]
REQUIRED_METHODS = ["get_action", "reset"]  # MyPolicy が実装すべきメソッド

# 展開時に無視/警告する不要物（パス構成要素で完全一致させる。部分一致は誤爆する）
JUNK_COMPONENTS = {".git", "__pycache__", ".venv", "venv", "node_modules",
                   ".DS_Store", ".ipynb_checkpoints", ".mypy_cache", ".pytest_cache"}

# requirements.txt で禁止するオプション（間接ファイル読込・editable・外部 index 等、
# 外部ソースを持ち込む指定。install 段階での通信・任意コード実行の温床）
BANNED_REQ_OPTIONS = ("-i", "--index-url", "--extra-index-url", "-f", "--find-links",
                      "-e", "--editable", "-r", "--requirement", "-c", "--constraint")
# 逆に、外部ソースを持ち込まない安全なオプションは許容する（誤検知防止）。
# --hash/--no-index はむしろサプライチェーン安全側。
SAFE_REQ_OPTIONS = ("--hash", "--require-hashes", "--no-index", "--pre",
                    "--no-binary", "--only-binary", "--prefer-binary")
# 禁止スキーム（PEP 508 direct reference や VCS 指定で外部ソースを引く）。
# pip は file:./x / file:/x を file:///x に正規化するため、スラッシュ数に依らず
# スキーム名で判定する（substring "file://" だけでは file:./x を取りこぼす）。
BANNED_REQ_SCHEMES = ("file", "http", "https", "ftp", "ftps", "ssh", "git", "hg",
                      "svn", "bzr")
# `git+https` のような VCS+トランスポート表記
_SCHEME_RE = re.compile(r"(?:^|[@\s=(])([a-z][a-z0-9]*(?:\+[a-z0-9]+)?)\s*:", re.I)

# 動的スモークで送るダミー観測のカメラ解像度（本番既定 128。pi05/GR00T は 256）
DEFAULT_CAMERA = 128
...
@dataclass
class Report:
    """バリデーション結果の集約。ERROR が1件でもあれば FAIL。"""
...
def _find_root_prefix(names: list[str]) -> tuple[str | None, str | None]:
    """必須ファイルが zip のルート直下にあるか、単一トップフォルダ配下かを判定。

    Returns:
        (prefix, note):
          prefix=""    → ルート直下に policy_server.py がある
          prefix="X/"  → 単一トップフォルダ X/ の配下にある（自動で1階層降りる）
          prefix=None  → 見つからない（呼び出し側でエラー化）
        note は警告メッセージ（該当時のみ）
    """
...
def validate_zip(zip_path: str | Path, report: Report | None = None) -> Report:
    ...
    with zf:
        ...
        # --- 重い testzip() の前に、メタデータだけで判定できる上限を先に検査する ---
        # （testzip は全エントリを完全解凍するため、これらより後に置くと悪意 zip で
        #   バリデータ自身が CPU/時間を枯渇させられる）

        # 暗号化 zip: 解凍不能なので testzip が RuntimeError を投げる。事前に弾く。
        ...
        # Zip Slip / symlink エントリ
        _check_zip_slip(infos, report)

        # zip bomb（展開後合計サイズ・圧縮率）— 中央ディレクトリのメタデータで判定
        ...
        # 上限超過が判明していれば、ここで全解凍(testzip)せず打ち切る
        if not report.ok:
            return report

        # 破損チェック（testzip は全解凍する。上限内と確認できてから呼ぶ）。
        # 暗号化以外の未知エラーでもゲートを落とさない。
        try:
            bad = zf.testzip()
            if bad is not None:
        ...
        # policy_server.py / requirements.txt の中身を読んで静的検査
...
def _check_zip_slip(infos: list[zipfile.ZipInfo], report: Report) -> None:
    """Zip Slip（パストラバーサル）と symlink エントリを検出。"""
    for info in infos:
        name = info.filename
        # 絶対パス / ドライブレター / UNC
        ...
        # symlink エントリ（Unix mode の上位ビットで判定）
...
def _check_policy_server_source(src: str, report: Report) -> None:
    """policy_server.py の静的検査: 構文・エンドポイント・メソッド・--port。"""
    import ast

    # 先頭 BOM を除去（Windows エディタ由来。実ファイル compile は BOM を無視するが
    # str の ast.parse は U+FEFF を構文エラー扱いするため、ここで揃える）
    src = src.lstrip("﻿")

    # 構文解析（Python バージョン非互換・文字コード事故の早期検出）。
    # null バイト等では SyntaxError でなく ValueError が飛ぶので両方捕捉する。
    try:
        ast.parse(src)
    except SyntaxError as e:
        report.error("policy.syntax",
                     f"policy_server.py に構文エラー: line {e.lineno}: {e.msg}")
        return  # 構文が壊れていれば以降の文字列チェックは無意味
    except ValueError as e:
        report.error("policy.syntax",
                     f"policy_server.py を解析できません（バイナリ/不正な文字の可能性）: {e}")
        return

    # 固定インターフェースのエンドポイント（文字列マッチ → 誤検知回避のため警告）
    ...
    # --port 引数対応（採点側は --port 8000 付きで起動する）。
    # 素の "port" 部分文字列は import 文等に必ず含まれ判定が死ぬので "--port" で見る。
    if "--port" not in src:
...
# ==================================================================
# §2 requirements.txt 依存チェック
# ==================================================================
def _split_option(token: str) -> str:
    """'--index-url' / '--index-url=...' / '-i' から先頭オプション名を取り出す。"""
    return token.split("=", 1)[0]


def check_requirements(text: str, report: Report) -> None:
    """requirements.txt の構文・外部ソース禁止を検査。

    設計方針:
      - 継続行 `\\` は pip と同じく**スペース無し**で結合する（`git\\`+`+https://`
        のような分割密輸を pip と同じ論理行で検査するため）。
      - 外部ソースはスキーム名（file/http/git/...）で判定する。`file:./x` のような
        非標準スラッシュ表記も pip は受理するため substring では取りこぼす。
      - `--hash`/`--no-index` 等の安全なオプションは許容し、誤検知を避ける。
    """
    # 先頭 BOM を除去（Windows エディタ由来）
    if text.startswith("﻿"):
        text = text[1:]

    # 継続行（行末 \）を pip 準拠でスペース無し結合
    ...
    # 大量の文字化け行の連鎖（バイナリ requirements）を1件に抑える
    syntax_errors = 0
    ...
    for ln, line in logical:
        ...
        # --- ローカルパス直接指定（bundled setup.py が install 時に実行される）---
        ...
        # --- 構文検証（packaging があれば。direct reference の URL も検出）---
        if have_packaging:
            try:
                ...
                if syntax_errors <= 10:
                    ...
                    report.error("req.syntax",
                                 ...
                                 "の可能性）。以降は省略します")
                continue
            # PEP 508 direct reference（name @ URL）の URL 部を一律禁止
...
def pip_dry_run(req_path: str | Path, report: Report, timeout: int = 300) -> None:
    """（任意）pip install --dry-run で依存解決可能かを事前確認。

    ネットワークアクセスを伴い、依存解決の backtracking で数分かかる場合がある
    ため既定では実行しない。CLI の --pip-dry-run で明示的に有効化する。
    """
    import subprocess
...
# ==================================================================
# 安全な展開（Zip Slip 対策つき extractall の代替）
# ==================================================================
def safe_extract(zip_path: str | Path, dest_dir: str | Path,
                 report: Report | None = None) -> Path:
    """Zip Slip / symlink を弾きつつ zip を dest_dir に展開する。

    evaluate.py の `zf.extractall` 直呼びの安全な置き換え。展開先を逸脱する
    エントリは黙って捨てず ValueError を送出する（採点環境への侵入経路を塞ぐ）。

    Returns:
        実質のルート（単一トップフォルダなら降りた先）
    """
...
def validate_dir(submission_dir: str | Path, report: Report | None = None) -> Report:
    ...
    # ルート直下判定（zip 経路と同じく、単一トップフォルダ包みは自動で1階層降りる）。
    # 展開済みディレクトリで自己診断する参加者が zip と矛盾する結論を得ないため。
...
def _make_dummy_obs(camera: int):
    """本番と同一キー・同一形状のダミー観測（テンプレ docstring 準拠）。"""
    import numpy as np
    return {
...
def _serialize_obs(obs) -> bytes:
    """remote_policy.serialize_obs と同一形式。"""
    import msgpack
    serialized = {
...
def smoke_test(submission_dir: str | Path, report: Report | None = None, *,
    ...
    """サーバーを起動し /health→/reset→/act の I/O を検証する（§3）。"""
    report = report or Report()
    d = Path(submission_dir)
    ...
    # start_new_session=True で新プロセスグループにする。参加者サーバーが推論ワーカー
    # 等の孫プロセスを起こしても、後始末でグループごと殺して孤児残留を防ぐ。
    ...
    descendants: list = []  # health 確認後に捕捉する子孫ツリー（孤児反映用）

    def _server_tail(n_bytes: int = 8192) -> str:
        # ログ末尾のみ読む（暴走サーバーの巨大ログでメモリスパイクしないため）
        try:
            log.flush()
    ...
    try:
        ...
        # サーバー健全時に子孫ツリーを捕捉（親が先に死んで孫が reparent されても
        # 確実に停止できるよう、生きているうちに掴んでおく）
        try:
            import psutil
        ...
        # --- /reset（本番クライアントと同じく instruction + seed を送る）---
        try:
        ...
        # --- 決定性（同一 seed・同一 obs で同一 action か）---
...
def _kill_process_group(proc, descendants=None) -> None:
    """サーバーとその全子孫を確実に停止する（孤児プロセス残留を防ぐ）。

    参加者サーバーが推論ワーカー等の子プロセスを起こすと、直接の子(uvicorn)を
    terminate しただけでは孫が reparent されて生き残る。psutil があれば起動時に
    捕捉した子孫ツリー(descendants)を個別に停止し、無ければプロセスグループ
    (start_new_session 前提)へ SIGTERM/SIGKILL する。
    """
    import signal
    import subprocess

    # 1) psutil 経路: 起動直後に捕捉した子孫 + 現在の子孫をまとめて停止
    ...
    try:
        ...
        for p in alive:
            try:
                ...
                pass
        return
    except Exception:  # noqa: BLE001 — psutil 不在/失敗時はプロセスグループ経路へ
        pass

    # 2) フォールバック: プロセスグループへシグナル（start_new_session 前提）
    try:
...
def _check_action_bytes(content: bytes, report: Report, camera: int,
                        quiet: bool = False):
    """/act の返り値を本番クライアント基準で検査。float32 shape(7,)・NaN/Inf・値域。"""
    import msgpack
    import numpy as np
    try:
    ...
    if action.shape != (7,):
        hint = ""
        # 本番は dtype をワイヤに乗せないため float32 で復元する。7要素を float64 で
        # 送ると 56バイト → float32 では 14要素に化ける。よくある取り違えを示唆する。
        if len(data) == 7 * 8:
...
def validate(target: str | Path, *, static_only: bool = False,
    ...
    """zip またはディレクトリを受け取り、静的（+動的）検査する統合エントリ。"""
    report = Report()
    target = Path(target)
    ...
    if target.is_file() and (zipfile.is_zipfile(target)
        ...
        validate_zip(target, report)  # 破損/切断 .zip もここで zip.corrupt を出す
        if do_pip_dry_run and report.ok:
            # requirements を取り出して dry-run
        ...
        if not report.ok:
            return report  # 静的で FAIL なら展開・起動しない
        if not static_only:
